...
class domain {
    ...
    public $id;  // ID of the domain to manipulate
    public $data;  // Domain & record details stored here
    public $format;  // "utf8" (default) or "idn" (punycode/IDN format)
    public $sql_obj;  // zone SQL database connection
...
//                $this->data["domain_name_utf"] = idn_to_utf8($this->data["domain_name"]);
...
// end of load_data_all
    ...
    public function load_data_record_all($exclude_forwardrev = NULL) {
...
// end of load_data_record_all
    ...
    public function load_data_record_custom($offset = 0, $limit = 0) {
...
// end of load_data_record_all
    ...
    public function data_record_custom_count() {
...
// end of action_update
    ...
    public function action_update_serial() {
...
// end of action_update_serial
    ...
    public function action_update_ns() {
...
// end of action_update_ns
    ...
    public function action_autofill_reverse() {
...
// end of action_autofill_reverse
    ...
    public function action_autofill_forward() {
...
class domain_records extends domain {
    ...
    public $id_record;  // ID of the domain record
    public $data_record; // domain record data
    ...
    public function verify_id_record() {
...
// end of verify_id_record
    ...
    public function find_forward_record($name) {
...
// end of find_forward_record
    ...
    public function find_reverse_domain($ip_address) {
...
// end of find_reverse_domain
    ...
    public function load_data_record() {
...
// end of action_update_record
    ...
    public function validate_custom_records($data_orig = []) {
...
//                $data_tmp[$i]["name"] = security_form_input_predefined("any", "record_custom_" . $i . "_name", 0, "");
        ...
        if (!empty($data_tmp)) {
            ...
            for ($i = 0; $i < $data["custom"]["num_records"]; $i++) {
                ...
                // verify name syntax
//                if ($data_tmp[$i]["name"] == "*" || preg_match("/^\*\.[A-Za-z0-9\p{L}:._-]+$/ui", $data_tmp[$i]["name"])) {
                if ($data_tmp[$i]["name"] == "*" || preg_match("/^\*\.[\\w\p{L}:._-]+$/ui", $data_tmp[$i]["name"])) {
                    ...
                    // nothing todo
//                } elseif ($data_tmp[$i]["name"] != "@" && !preg_match("/^[A-Za-z0-9\p{L}:._-]*$/ui", $data_tmp[$i]["name"])) {
