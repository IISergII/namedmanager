...
class template_engine {
    ...
    public $data;  // data for standard fields
    public $data_array; // data for array field	
    public $data_files; // data for files
    public $template;  // template contents
    public $processed;  // processed lines of the templates (template with field data merged)
    public $output;  // field to hold processed information (eg: binary file data)
    ...
    public function prepare_load_template($templatefile) {
    ...
    public function prepare_add_field($fieldname, $value) {
    ...
    public function prepare_add_file($fieldname, $file_extension, $file_type, $file_id) {
    ...
    public function prepare_add_array($fieldname, $structure) {
    ...
    public function fillter_template_data() {
    ...
    public function process_template_loops($line, $data, $fieldname, $fieldnames, $level) {
            ...
            if ($current_level[$fieldname] != null) {
                foreach ($current_level[$fieldname] as $key => $current_level_part) {
                    if (preg_match("/^\S*\sif\s(\S*)\sne\sprevious\s-->/", $line, $matches)) {
                        $line_tmp = '';
                        $match = $matches[1];
                        if ((!isset($current_level[$fieldname][($key - 1)][$match]) && $current_level_part[$match]) || ($current_level_part[$match] != $current_level[$fieldname][($key - 1)][$match])) {
                            $line_tmp = str_replace($matches[0], '', $line);
                        } else { // only print what is after the endif
    ...
    public function prepare_filltemplate() {
        log_debug("template_engine", "Executing prepare_filltemplate()");

        $fieldname = "";
        $fieldnames = array(0 => '');
        $repeated_processed_lines = [];
                    ...
                    if ($in_foreach > 1) {
                        ...
                        for ($j = 0; $j < count($this->data_array[$parent_fieldname]); $j++) {
                            if (empty($repeated_processed_lines[$j])) {
                                $repeated_processed_lines[$j] = [];
                            }

                            if (empty($repeated_processed_line_sections[$j])) {
                                $repeated_processed_line_sections[$j] = [];
                            }
...
class template_engine_latex extends template_engine {
    ...
    public function prepare_escape_fields() {
...
class template_engine_htmltopdf extends template_engine {
    ...
    public function set_template_directory($template_dir) {
